...
public abstract class ArbolBinario<T> implements Coleccion<T> {
     ...
     */
    @Override public String toString() {
	    return (raiz == null) ? "" : toString(raiz, 0, new boolean[raiz.altura() + 1]);
    }
     ...
     */
    private String toString(Vertice v, int l, boolean[] bin){
     ...
     * @return los espacios.
     */
    private String dibujaEspacios(int l, boolean[] bin) {
        StringBuffer sb = new StringBuffer();
        for (int i = 0; i <= l - 1; i++)
            if (bin[i])
                sb.append("│  ");
